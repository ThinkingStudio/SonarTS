...
export class Rule extends tslint.Rules.AbstractRule {
  ...
  private static message(functionBlock: ts.Block): string {
    const lineOfOriginalFunction = lineAndCharacter(functionBlock.parent!.getStart(), functionBlock.getSourceFile())
      .line;
...
class Walker extends tslint.RuleWalker {
  ...
  protected visitNode(node: ts.Node): void {
    if (is(node, ...FUNCTION_LIKE)) {
      ...
      if (is(body, ts.SyntaxKind.Block) && (body as ts.Block).statements.length > 1) {
